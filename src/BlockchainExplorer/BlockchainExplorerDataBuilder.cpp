...
#include "GenerationXCore/GenerationXTools.h"
#include "GenerationXCore/TransactionExtra.h"
#include "GenerationXConfig.h"

namespace GenerationX {

BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(GenerationX::ICore& core, GenerationX::IGenerationXProtocolQuery& protocol) :
core(core),
protocol(protocol) {
}
...
bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails& blockDetails) {
  ...
  if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes, parameters::GENERATIONX_REWARD_BLOCKS_WINDOW)) {
    return false;
  }

This is the reference code for [GenerationX](https://genxcoin.org) cryptocurrency protocol.

* Discussion board and support: [GenerationX Forum](https://forum.genxcoin.org)

## GenerationX forking how-to

### Preparation

1. Create an account on [GitHub.com](github.com)
2. Fork [GenerationX repository]
...
**1. in file src/GenerationXConfig.h** - `GENERATIONX_NAME` constant

Example: 
```
const char GENERATIONX_NAME[] = "furiouscoin";
```
...
**1. Total money supply** (src/GenerationXConfig.h)

Total amount of coins to be emitted. Most of GenerationX based coins use `(uint64_t)(-1)` (equals to 18446744073709551616). You can define number explicitly (for example `UINT64_C(858986905600000000)`).
...
**2. Emission curve** (src/GenerationXConfig.h)

Be default GenerationX provides emission formula with slight decrease of block reward with each block. This is different from Bitcoin where block reward halves every 4 years.
...
**3. Difficulty target** (src/GenerationXConfig.h)
...
In case you are not satisfied with GenerationX default implementation of block reward logic you can also change it. The implementation is in `src/GenerationXCore/Currency.cpp`:
```
...
- big block penalty calculation: this is the way GenerationX protects the block chain from transaction flooding attacks and preserves opportunities for organic network growth at the same time.
...
**1. Default ports for P2P and RPC networking** (src/GenerationXConfig.h)
...
```
const static boost::uuids::uuid GENERATIONX_NETWORK = { { 0xA1, 0x1A, 0xA1, 0x1A, 0xA1, 0x0A, 0xA1, 0x0A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA1, 0x1A } };
```


**3. Seed nodes** (src/GenerationXConfig.h)
...
**1. Minimum transaction fee** (src/GenerationXConfig.h)
...
**2. Penalty free block size** (src/GenerationXConfig.h)

GenerationX protects chain from tx flooding by reducing block reward for blocks larger than the median block size. However, this rule applies for blocks larger than `GENERATIONX_BLOCK_GRANTED_FULL_REWARD_ZONE` bytes.

Example:
```
const size_t GENERATIONX_BLOCK_GRANTED_FULL_REWARD_ZONE = 20000;
```


### Fifth step. Address prefix

You may choose a letter (in some cases several letters) all the coin's public addresses will start with. It is defined by `GENERATIONX_PUBLIC_ADDRESS_BASE58_PREFIX` constant. Since the rules for address prefixes are nontrivial you may use the [prefix generator tool](https://GenerationXstarter.org/tools.html).

Example:
```
const uint64_t GENERATIONX_PUBLIC_ADDRESS_BASE58_PREFIX = 0xe9; // addresses start with "f"
```


### Sixth step. Genesis block

**1. Build the binaries with blank genesis tx hex** (src/GenerationXConfig.h)
...
**3. Copy the printed transaction hash** (src/GenerationXConfig.h)

Copy the tx hash that has been printed by the daemon to `GENESIS_COINBASE_TX_HEX` in `src/GenerationXConfig.h`
